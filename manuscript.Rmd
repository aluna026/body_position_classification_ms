...
Infants were recruited in one of two age groups: Younger infants were 4-7 months and older infants were 11-14 months. Families were recruited through social media advertisements and from community events in Southern California. There were `r num_younger` infants in the 4-7 month group (X female) and `r num_older` in the 11-14 month group (X female). Only 1 of session was excluded due to a technical error---one of the four IMU sensors failed to record, resulting in an unusable recording. The ethnicity for infants was reported as X Hispanix/Latinx and X for not Hispanix/Latinx. Race was reported as X Caucasian, X Asian, X ~~~ and X others. Parents were compensated \$30 in cash for each visit they completed. The BLINDED Institutional Review Board reviewed and approved all procedures associated with the study. All participants gave their informed consent to participate.
<!-- TODO RACE ETHNICITY SEX -->

Most participants were tested in a single session (`r nrow(num_sessions %>% filter(n == 1))`), but `r nrow(num_sessions %>% filter(n > 1))` participants contributed between 2-4 sessions as part of an ongoing longitudinal study. Across the two age groups, we report data on a total of `r length(unique(ppts$id_uni))` sessions, with `r num_sessions %>% filter(group == "Younger") %>% pull(n) %>% sum` from younger infants and `r num_sessions %>% filter(group == "Older") %>% pull(n) %>% sum` from older infants. Across sessions, younger infants' age ranged from `r age["Younger","min"]` to `r age["Younger","max"]` (*M* = `r age["Younger","mean"]`) and older infants' age ranged from `r age["Older","min"]` to `r age["Older","max"]` (*M* = `r age["Older","mean"]`).
