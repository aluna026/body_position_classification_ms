...
model <- pt2_group %>% count(file, pos) %>% add_count(file, wt = n, name = "total") %>% mutate(prop_model = n/total)
human <- pt2_group %>% count(file, code) %>% add_count(file, wt = n, name = "total") %>% mutate(prop_human = n/total)
...
group_agree_cor <- cor_test(agree_group, vars = c("prop_model", "prop_human")) %>% pull(cor)
...
ggplot(agree_group) +
  ...
  scale_x_continuous(breaks = c(0,.25, .5, .75, 1), labels = c(0, 25, 50, 75, 100), limits = c(0, 1.07), name = "Actual Position Prevalence (%)") + 
  scale_y_continuous(breaks = c(0,.25, .5, .75, 1), labels = c(0, 25, 50, 75, 100), limits = c(0, 1.07), name = "Predicted Position Prevalence (%)") + 
