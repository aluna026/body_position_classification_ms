...
#   ds %>% mutate(diff = prop_human - prop_model) %>% 
#   ggplot(aes(x = pos, y = diff, color = pos)) +
#   stat_summary() + 
#   scale_fill_manual(values = pal, guide = "none") + 
#   ylab("Prediction Difference (%)") + ylim(-.5, .5) + xlab("")
# }
```

```{r part2overall, warning=FALSE, fig.cap="Overall agreement between human-coded body position and model-predicted body position in the long-delay period. Agreement for group models is shown in (A) and agreement for individual models is shown in (B). Plots are shown separately for each body position with a reference line that indicates perfect agreement; each point in a plot represent data for a single participant.", fig.align="center", fig.width=7, fig.height=6.5}
...
pt2corr_without_outliers <- map_dfc(agree, ~ .x %>% filter(file != "107/3", file != "104/1") %>% group_by(pos) %>% cor_test(vars = c("prop_model", "prop_human")) %>% select(pos, cor)) %>% select(-`pos...1`,-`pos...3`)
...
group_agree_cor_noout <- agree$group %>% filter(file != "107/3", file != "104/1") %>% 
  cor_test(vars = c("prop_model", "prop_human")) %>% pull(cor)
split_agree_cor_noout <- agree$split %>% filter(file != "107/3", file != "104/1") %>%
  cor_test(vars = c("prop_model", "prop_human")) %>% pull(cor)
...
# difference_plots <- map(agree, difference_plot)
...
pt2corr_without_outliers <- map_dfc(agree_bins, ~ .x %>% filter(file != "107/3", file != "104/1") %>% group_by(pos) %>% cor_test(vars = c("prop_model", "prop_human")) %>% select(pos, cor)) %>% select(-`pos...1`,-`pos...3`)
pt2corr <- bind_cols(pt2corr_with_outliers, pt2corr_without_outliers)
rm(pt2corr_with_outliers)
rm(pt2corr_without_outliers)
...
group_agree_cor_noout <- agree_bins$group %>% filter(file != "107/3", file != "104/1") %>% 
  cor_test(vars = c("prop_model", "prop_human")) %>% pull(cor)
split_agree_cor_noout <- agree_bins$split %>% filter(file != "107/3", file != "104/1") %>%
  cor_test(vars = c("prop_model", "prop_human")) %>% pull(cor)
overall_row <- tibble("Overall", group_agree_cor, split_agree_cor, group_agree_cor_noout, split_agree_cor_noout)
colnames(overall_row) <- colnames(pt2corr)
pt2corr <- bind_rows(pt2corr, overall_row)
...
# individual_scatter <- function(ds, pos_select) {
#   ds %>% filter(pos == pos_select) %>% ggplot() +
#   geom_abline(intercept = 0, slope = 1) + facet_wrap("file") + 
#   geom_point(aes(x = prop_human, y = prop_model, fill = pos), shape = 21, alpha = .5, size = 4) + 
#   scale_fill_manual(values = pal, guide = "none") + 
#   scale_x_continuous(breaks = c(0,.5,1), labels = c(0, 50, 100), limits = c(-.07, 1.07), name = "Actual Prevalence (%)") + 
#   scale_y_continuous(breaks = c(0,.5,1), labels = c(0, 50, 100), limits = c(-.07, 1.07), name = "Predicted Prevalence (%)") + 
...
# individual_scatter <- function(ds) {
#   ds %>% ggplot() +
#   geom_abline(intercept = 0, slope = 1) + facet_grid(c("file","pos")) + 
#   geom_point(aes(x = prop_human, y = prop_model, fill = pos), shape = 21, alpha = .5, size = 3) + 
#   scale_fill_manual(values = pal, guide = "none") + 
#   scale_x_continuous(breaks = c(0,.5,1), labels = c(0, 50, 100), limits = c(-.07, 1.07), name = "Actual Prevalence (%)") + 
#   scale_y_continuous(breaks = c(0,.5,1), labels = c(0, 50, 100), limits = c(-.07, 1.07), name = "Predicted Prevalence (%)") + 
...
# }
# individual_scatter(agree_bins$group)

# agree_plots_bins <- map(agree_bins, ~ individual_scatter(.x, "Prone"))
# agree_plots_bins <- map2(agree_plots_bins, c("A. Group Models", "B. Individual Models"), ~ .x + ggtitle(.y))
# agree_plots_bins[[1]] / agree_plots_bins[[2]]
# (individual_scatter(agree_bins$group, "Sitting") + individual_scatter(agree_bins$group, "Upright"))/
#   (individual_scatter(agree_bins$group, "Prone") + individual_scatter(agree_bins$group, "Supine"))
# 
