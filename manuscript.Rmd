...
```{r age, warning=FALSE, fig.cap="Age trends", fig.align="center", fig.width=5, fig.height=8}
fullday <- read_csv("data/imu-compiled.csv") %>% 
  mutate(id_uni = factor(id*100+session),
         group = factor(ifelse(age <= 8, "Younger", "Older")),
         nap_time = ifelse(is.na(sit_time), nap_time, NA))
fullday_long <- fullday %>% pivot_longer(cols = sit_time:upright_time, names_to = "position", values_to = "prop") %>% 
  mutate(position = factor(position,
...
fullday_long %>% drop_na(prop) %>% 
  ggplot() + facet_wrap("position", as.table = T, ncol = 2) + 
  geom_smooth(aes(x = age, y = prop), color = "darkgray", method = "lm", se = T) + 
  stat_summary(fun = "mean", mapping = aes(x = age, y = prop, group = id_uni, fill = position), shape = 21, size = .75) + 
  scale_fill_manual(values = pal, guide = "none") + 
